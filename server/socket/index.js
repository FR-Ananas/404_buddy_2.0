const { roomQueries, messageQueries, userQueries } = require("../db");

// roomName → Set of socket ids
const roomMembers = new Map();

// socketId → { userId, username, avatar, roomName }
...
function getUsersInRoom(roomName) {
  ...
  return [...members].map((sid) => {
    ...
    return u ? { id: u.userId, username: u.username, avatar: u.avatar } : null;
  }).filter(Boolean);
}
...
module.exports = function registerSocketHandlers(io) {
  io.on("connection", (socket) => {
    ...
    socket.on("join_room", (roomName) => {
      ...
      socketUsers.set(socket.id, { userId, username, avatar, roomName });

      // Send history
      const history = messageQueries.lastN.all(room.id, 50).reverse();
      socket.emit("history", history);

      // Announce & broadcast user list
    ...
    socket.on("send_message", ({ content, type = "text" }) => {
      ...
      if (type === "image" && content.length > 4 * 1024 * 1024) return; // 4MB base64 limit
      ...
      io.to(user.roomName).emit("new_message", message);
    });
